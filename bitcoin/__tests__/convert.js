...
test('Allow to remove a unit' , () => {
  convert.removeUnit('BTC');
  expect(convert.units()).toHaveLength(5);
});

test('Cannot remove undefined units' , () => {
  expect(() => {
    convert.removeUnit('MOC');
  }).toThrow();
});

test('Allow adding a unit' , () => {
  convert.addUnit('MEX' , new Big(2));
  expect(convert.units()).toHaveLength(6);
});
